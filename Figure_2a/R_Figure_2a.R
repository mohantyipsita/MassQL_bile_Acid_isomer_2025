...
######################################################################################
##############Identifying Bile acid MS/MS spectra with the diagnostic ions############
######################################################################################
#check how many of the MS/MS spectra were captured in the mono,di and trihydroxy stage1 query
monohydroxy_scans_stage1 <- unlist(df_comb_edit$scan_list[df_comb_edit$query == "Monohydroxy_stage1"])
dihydroxy_scans_stage1 <- unlist(df_comb_edit$scan_list[df_comb_edit$query == "Dihydroxy_stage1"])
trihydroxy_scans_stage1 <- unlist(df_comb_edit$scan_list[df_comb_edit$query == "Trihydroxy_stage1"])

# Combine all the character vectors into one
all_scans <- c(monohydroxy_scans_stage1, dihydroxy_scans_stage1, trihydroxy_scans_stage1)

df_feature_req_BA <- df_feature[df_feature$row_ID %in% names(df_merge_BA_meta_2), ]
df_feature_req_BA_sub <- df_feature_req_BA[,c(1,2,3,7)]

# Subset the dataframe to keep only those scans that have the two ions
df_feature_req_BA_sub_diag <- df_feature_req_BA_sub %>%
  filter(row_ID %in% all_scans)

##add back the bile acid scans from any other library than the GNPS candidate library
...
df_libhit_BA_noncandidateBAs <- df_libhit_BA %>%
  filter(Organism != "GNPS-BILE-ACID-MODIFICATIONS")
# Subset rows from df_feature_req_BA_sub where row_ID is present in the Scan column of df_libhit_BA_noncandidateBAs
df_feature_req_BA_sub2 <- df_feature_req_BA_sub %>%
  filter(row_ID %in% df_libhit_BA_noncandidateBAs$Scan)
# Append the subsetted rows from df1 to df2
df_feature_req_BA_sub_diag2 <- bind_rows(df_feature_req_BA_sub_diag, df_feature_req_BA_sub2)

#########################################################
###############IIMN Merging##############################
#########################################################

#The next code snippet is to identify unique bile acid modifications by combining ISFs, adduct forms and features with same RT

unique_BAs <- unique(df_feature_req_BA_sub_diag2$`correlation group ID`) #gave 124 correlation groups
num_NAs <- sum(is.na(df_feature_req_BA_sub_diag2$`correlation group ID`)) #gave 477 that were not assigned into any correlation group

######################################################################################
######################################################################################


###subsetting the df_merge_BA_meta_2 for only the filtered MS/MS scans for volcano plot
df_merge_BA_meta_2_diag <- df_merge_BA_meta_2 %>%
  dplyr::select(1, 2, which(colnames(df_merge_BA_meta_2) %in% df_feature_req_BA_sub_diag2$row_ID))
#write.csv(df_merge_BA_meta_2_diag, "MetaboanalystR/Input_final_metaboanalystR_new_diag.csv", row.names = FALSE)
...
mSet<-Read.TextData(mSet, "MetaboanalystR/Input_final_metaboanalystR_new_diag.csv", "rowu", "disc")
...
df_foldchange <- read.csv("MetaboanalystR/FINAL_DIAG/fold_change.csv", check.names = FALSE)
df_ttest <- read.csv("MetaboanalystR/FINAL_DIAG/t_test_all.csv", check.names = FALSE)
...
ggsave("FINAL_volcano_plot_di_diag.svg", plot = volcano_plot_di, width = 10, height = 8, units = "in", device = "svg")
ggsave("FINAL_volcano_plot_di_diag.png", plot = volcano_plot_di, width = 10, height = 8, units = "in", device = "png")


##########################################################################
########################Figure Supplementary XXXX#########################
##########################################################################

df_volcano_all_sub_annot_all_class <- df_volcano_all_sub_annot %>%
  mutate(
    class_all = case_when(
      Scan %in% monohydroxy_scans_stage1 ~ "Mono",
      Scan %in% dihydroxy_scans_stage1 ~ "Di",
      Scan %in% trihydroxy_scans_stage1 ~ "Tri",
      TRUE ~ NA_character_  # Fill with NA if none of the conditions are met
    )
  )


# Create a named vector of colors for each class
class_colors <- c("Mono" = "#4EB748", "Di" = "#F8962E", "Tri" = "#F6EB16", "NA" = "#758694")


# Create a named vector of shapes for each class
class_shapes <- c("Mono" = 18, "Di" = 18, "Tri" = 18, "NA" = 1)


# Apply the colors based on the class column
df_volcano_all_sub_annot_all_class$color <- class_colors[df_volcano_all_sub_annot_all_class$class_all]

df_volcano_all_sub_annot_all_class$shape <- class_shapes[df_volcano_all_sub_annot_all_class$class_all]


# Check if lengths match
stopifnot(length(df_volcano_all_sub_annot_all_class$color) == nrow(df_volcano_all_sub_annot_all_class))
stopifnot(length(df_volcano_all_sub_annot_all_class$shape) == nrow(df_volcano_all_sub_annot_all_class))


# Creating a keyvals vector similar to the documentation example
keyvals <- df_volcano_all_sub_annot_all_class$color
names(keyvals) <- df_volcano_all_sub_annot_all_class$class_all

# Creating a named vector for shapes similar to the documentation example
shape_vals <- df_volcano_all_sub_annot_all_class$shape
names(shape_vals) <- df_volcano_all_sub_annot_all_class$class_all

# Create a custom size vector
point_size <- ifelse(df_volcano_all_sub_annot_all_class$class_all == "NA", 0.8, 3.5)

# Check the custom size vector
table(point_size)

# Create a vector of labels to display (those that are not "NA" in class)
labels_to_display <- df_volcano_all_sub_annot_all_class$Scan[df_volcano_all_sub_annot_all_class$class_all != "NA"]


#Plotting the volcano plot with EnhancedVolcano plot
library(EnhancedVolcano)

volcano_plot_all <- EnhancedVolcano(df_volcano_all_sub_annot_all_class,
                                   lab = df_volcano_all_sub_annot$Scan,
                                   x = 'log2FC',
                                   y = 'FDR',
                                   title = 'Fecal vs Instestine',
...
# Save the plot as SVG
ggsave("FINAL_volcano_plot_all.svg", plot = volcano_plot_all, width = 10, height = 8, units = "in", device = "svg")
ggsave("FINAL_volcano_plot_all.png", plot = volcano_plot_all, width = 10, height = 8, units = "in", device = "png")
