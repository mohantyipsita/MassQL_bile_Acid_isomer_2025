...
######################################################################################
#check how many of the 937 MS/MS spectra were captured in the mono,di and trihydroxy stage1 query
...
unique_BAs <- unique(df_feature_req_BA_sub_diag$`correlation group ID`) #gave 115 correlation groups
num_NAs <- sum(is.na(df_feature_req_BA_sub_diag$`correlation group ID`)) #gave 392 that were not assigned into any correlation group
...
df_merge_BA_meta_2_diag <- df_merge_BA_meta_2 %>%
  dplyr::select(1, 2, which(colnames(df_merge_BA_meta_2) %in% all_scans))
