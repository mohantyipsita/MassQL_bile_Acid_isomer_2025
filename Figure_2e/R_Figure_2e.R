...
df_long_mind <- df_mind_BLY1 %>%
  select(ID, BL_58941, T1_58941, delta_58941) %>%
...
df_wide_mind_nonzero <- df_long_mind_nonzero %>%
  select(ID, Timepoint, Peak_Area) %>%
  pivot_wider(names_from = Timepoint, values_from = Peak_Area) %>%
  filter(!is.na(BL), !is.na(Y1))  # Ensure no missing values for BL or T3

# Perform a Wilcoxon signed-rank test on the paired data (BL vs Y1)  ----- not considering it PAIRED
wilcox_test_result <- wilcox.test(df_wide_mind_nonzero$BL, df_wide_mind_nonzero$Y1)
# View the p-value
wilcox_test_result$p.value

# Perform a Wilcoxon test ---- considering it PAIRED
